...
//			calibrateSimple();
    ...
    while (false == miniwayFind(&maze, start_coordinate, zhonxSettings.maze_end_coordinate));
...
void moveRealZhonxArc(labyrinthe *maze, positionRobot *positionZhonx, coordinate way[])
{
    ...
    while (way[i].x != END_OF_LIST)
    {
        ...
        {
            bluetoothPrintf("Error way : position zhonx x= %d y=%d \t way x= %d y=%d \n", positionZhonx->cordinate.x,
                            positionZhonx->cordinate.y, way[i].x, way[i].y);
            HAL_Delay(200);
            motorsDriverSleep(ON);
            ssd1306ClearScreen(MAIN_AREA);
        ...
        for (x = 0; x < MAZE_SIZE; x++)
        {
            if (maze.cell[x][y].wall_north == WALL_PRESENCE)
            {
                ssd1306DrawLine(x * size_cell_on_oled, (y + DISPLAY_OFFSET / 4) * size_cell_on_oled,
                                ...
                                (y + DISPLAY_OFFSET / 4) * size_cell_on_oled);
            }
...
//                x * size_cell_on_oled + size_cell_on_oled + 1, y * size_cell_on_oled);
            ...
            if (maze.cell[x][y].wall_west == WALL_PRESENCE)
            {
                ssd1306DrawLine(x * size_cell_on_oled, (y + DISPLAY_OFFSET / 4) * size_cell_on_oled,
                                x * size_cell_on_oled,
                                (y + DISPLAY_OFFSET / 4) * size_cell_on_oled + size_cell_on_oled + 1);
            }
            ...
            if (maze.cell[x][y].wall_south == WALL_PRESENCE)
            {
                ssd1306DrawLine(x * size_cell_on_oled, ((y + DISPLAY_OFFSET / 4) + 1) * size_cell_on_oled,
                                ...
                                ((y + DISPLAY_OFFSET / 4) + 1) * size_cell_on_oled);
            }
    ...
    ssd1306DrawRect((robot_coordinate.x * size_cell_on_oled) + 1,
                    ((robot_coordinate.y + (DISPLAY_OFFSET / 4)) * size_cell_on_oled) + 1, 2, 2);
    ssd1306Refresh();
#endif
}
