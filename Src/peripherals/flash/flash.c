 ...
 * - use 'buffer = alloca(ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE)' in flash_read and flash_write
...
/* local helpers */
static int check_addr_range(FLASH_INTERNAL_HANDLE *ihandle, uintptr_t p, unsigned int len);
// factorize code for flash_erase and flash_write. flash_read differs
static int update_flash_with_buffer(FLASH_INTERNAL_HANDLE *ihandle, uintptr_t p, unsigned int length, unsigned char *buf);
...
int flash_open(FLASH_DRIVER_DESC *flash_driver_desc, FLASH_HANDLE *handle)
{
    ...
    flash_driver_desc->ops.flash_driver_init();
    
    /* copy the provided driver descriptor */
    memcpy(&(flash_internal_handles[i].drv_desc), flash_driver_desc, sizeof(FLASH_DRIVER_DESC));
                    ...
                    unsigned int   length)
{
    ...
    rv = update_flash_with_buffer(ihandle, (uintptr_t)p_dest, length, NULL/*no buffer, just erase*/);
                    ...
                    unsigned int   length)
{
    ...
    if ( ((uintptr_t)p_src >= ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE)
            && ((uintptr_t)p_src < (ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE + ihandle->drv_desc.props.FLASH_DRIVER_FLASH_SIZE)) )
    {
        TRACE("ERROR: src pointer is in Flash");
        return FLASH_E_WRONG_PARAMS;
    }

    rv = update_flash_with_buffer(ihandle, (uintptr_t)p_dest, length, p_src);
                   ...
                   unsigned int   length)
{
    ...
    sector_mask = get_align_mask(ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE);
    ...
    if ( ((uintptr_t)p_dest >= ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE)
            && ((uintptr_t)p_dest < (ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE + ihandle->drv_desc.props.FLASH_DRIVER_FLASH_SIZE)) )
    {
    ...
    num_sectors = (end_sector_addr - base_sector_addr)/ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE + 1;
     ...
     */
    aligned_p        = ptr_align(p, ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE);
    // starting offset
    base_offset      = p & ~(sector_mask);
    // quantum aligned, backwards
    base_offset_algn = (aligned_p == p) ? base_offset : ((aligned_p - ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE) & ~(sector_mask));
    // ending offset
    ...
    end_bytes_to_read_algn = ptr_align((uintptr_t)(end_offset+1), ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE) % ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE; // not a pointer, but same operation
    ...
    if (base_offset)
    {
        ...
        memset(flash_sector_buffer, ihandle->drv_desc.props.FLASH_DRIVER_VIRGIN_BYTE, sizeof(flash_sector_buffer));
        ...
        rv = ihandle->drv_desc.ops.flash_driver_sector_read_buf(base_sector_addr,
                ...
                flash_sector_buffer,
                ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - base_offset_algn);
        if (rv != FLASH_DRIVER_E_SUCCESS)
        {
            TRACE3("ERROR: flash driver failed at reading %u bytes at 0x%0X with error code 0x%0X",
                    ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - base_offset_algn, base_sector_addr + base_offset_algn, rv);
            return FLASH_E_DRIVER_ERROR;
        }
        else
        {
            TRACE2("flash driver read %u bytes at 0x%0X",
                    ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - base_offset_algn, base_sector_addr + base_offset_algn);
        }

        /* copy the data into the destination buffer, skipping any additional data present due to alignment */
        size = MIN(ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - base_offset, length);
        memcpy(p_dest,
        ...
        base_sector_addr += ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE;
        num_sectors--;
    ...
    if (end_bytes_to_read_algn)
    {
        ...
        end_sector_addr -= ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE;
        num_sectors--;
    }

    for (i = 0; i < num_sectors; ++i, base_sector_addr += ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE, p_dest += ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE)
    {
        rv = ihandle->drv_desc.ops.flash_driver_sector_read_buf(base_sector_addr,
                0,
                p_dest,
                ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE);
        if (rv != FLASH_DRIVER_E_SUCCESS)
        {
...
static int check_addr_range(FLASH_INTERNAL_HANDLE *ihandle, uintptr_t p, unsigned int len)
{
    ...
    if (ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE > p)
    {
        TRACE2("ERROR: address 0x%0X is below flash area (starts at 0x%0X)", p, ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE);
        return FLASH_E_UNDERFLOW;
    }
    if ( (ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE + ihandle->drv_desc.props.FLASH_DRIVER_FLASH_SIZE) < (p + len))
    {
        TRACE2("ERROR: end address 0x%0X is beyond flash area (ends at 0x%0X)",
                p + len,
                ihandle->drv_desc.props.FLASH_DRIVER_FLASH_BASE + ihandle->drv_desc.props.FLASH_DRIVER_FLASH_SIZE);
...
static int update_flash_with_buffer(FLASH_INTERNAL_HANDLE *ihandle, uintptr_t p, unsigned int length, unsigned char *buf)
{
    ...
    uintptr_t sector_mask = get_align_mask(ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE);
     ...
     * For things to work properly, align addresses and erase the little extra in the RAM buffer
     */
    aligned_end      = ptr_align((p + length - 1), ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE) - ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE; // quantum aligned, backwards
    // starting offset
    base_offset      = p & ~(sector_mask);
    // ending offset
    end_offset       = (p + length - 1) & ~(sector_mask);
    ...
    base_bytes_to_save_algn = ptr_align((uintptr_t)base_bytes_to_save, ihandle->drv_desc.props.FLASH_DRIVER_QUANTUM_SIZE);//not a pointer but the operation is the same
    end_bytes_to_save       = ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - end_offset - 1/*end_offset is the offset of the last erased/updated cell*/;
    ...
    if (num_sectors == 1)
    {
        ...
        if (base_bytes_to_save)
        {
            ...
            {
                memset(flash_sector_buffer + base_offset, ihandle->drv_desc.props.FLASH_DRIVER_VIRGIN_BYTE, base_bytes_to_save_algn - base_bytes_to_save);
            }
        }
        ...
        if (end_bytes_to_save)
        {
            rv = ihandle->drv_desc.ops.flash_driver_sector_read_buf(end_sector_addr/*== base_sector_addr*/,
                    ...
                    ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - end_offset_algn);
            if (rv != FLASH_DRIVER_E_SUCCESS)
            {
                TRACE3("ERROR: flash driver failed at reading %u bytes at 0x%0X with error code 0x%0X",
                        ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - end_offset_algn, end_sector_addr + end_offset_algn, rv);
                return FLASH_E_DRIVER_ERROR;
            }
            else
            {
                TRACE2("flash driver read %u bytes at 0x%0X",
                        ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - end_offset_algn, end_sector_addr + end_offset_algn);
            }
            /* remove any additional data copied due to alignment corrections */
            memset(flash_sector_buffer + end_offset_algn, ihandle->drv_desc.props.FLASH_DRIVER_VIRGIN_BYTE, end_offset - end_offset_algn + 1);
        }
    ...
    else
    {
        ...
        if (base_bytes_to_save)
        {
            ...
            if (buf != NULL)
            {
                size = ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE - base_offset;
         ...
         */
        for (i = 0; i < num_sectors; ++i, base_sector_addr += ihandle->drv_desc.props.FLASH_DRIVER_SECTOR_SIZE)
        {
            rv = ihandle->drv_desc.ops.flash_driver_sector_erase(base_sector_addr);
            if (rv != FLASH_DRIVER_E_SUCCESS)
            {
