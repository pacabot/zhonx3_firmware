...
int
wallSensorsCalibrationFront (void)
{
  ...
  for (i = 0; i < TELEMETER_PROFILE_ARRAY_LENGTH; i++)
    {
      ...
      ssd1306PrintIntAtLine (0, 3, "FL",
                             (uint32_t) getTelemeterAvrg (TELEMETER_FL),
                             &Font_5x8);
      ssd1306PrintIntAtLine (0, 4, "FR",
                             (uint32_t) getTelemeterAvrg (TELEMETER_FR),
                             &Font_5x8);
  ...
  for (int i = 0; i < TELEMETER_PROFILE_ARRAY_LENGTH; i++)
    {
      bluetoothPrintf ("%d|%d|%d\n", i, front_telemeters.left[i],
                       front_telemeters.right[i]);
    }
...
int
wallSensorsCalibrationDiag (void)
{
  ...
  double number_of_millimeter_per_loop = sqrt (
      2 * pow (NUMBER_OF_MILLIMETER_BY_LOOP, 2));
  telemetersStart ();
  // take the measures
  move (0, -number_of_millimeter_per_loop * TELEMETER_PROFILE_ARRAY_LENGTH, 5,
        5);
  for (i = 0; i < TELEMETER_PROFILE_ARRAY_LENGTH; i++)
    {
      while ((((int) (encoderGetDist (ENCODER_L) + encoderGetDist (ENCODER_R))
      ...
      ssd1306PrintIntAtLine (0, 3, "DL",
                             (uint32_t) getTelemeterAvrg (TELEMETER_DL),
                             &Font_5x8);
      ssd1306PrintIntAtLine (0, 4, "DR",
                             (uint32_t) getTelemeterAvrg (TELEMETER_DR),
                             &Font_5x8);
  ...
  for (int i = 0; i < TELEMETER_PROFILE_ARRAY_LENGTH; ++i)
    {
      bluetoothWaitReady ();
      bluetoothPrintf ("%d|%d|%d\n", i, diag_telemeters.left[i],
                       diag_telemeters.right[i]);
    }
  ...
  while (expanderJoyFiltered () != JOY_LEFT)
    {
      ...
      ssd1306PrintIntAtLine (
          60, 1, "FL ", (uint32_t) (getTelemeterDist (TELEMETER_FL) * 10.00),
          &Font_5x8);
      ssd1306PrintIntAtLine (
          60, 2, "DL ", (uint32_t) (getTelemeterDist (TELEMETER_DL) * 10.00),
          &Font_5x8);
      ssd1306PrintIntAtLine (
          60, 3, "DR ", (uint32_t) (getTelemeterDist (TELEMETER_DR) * 10.00),
          &Font_5x8);
      ssd1306PrintIntAtLine (
          60, 4, "FR ", (uint32_t) (getTelemeterDist (TELEMETER_FR) * 10.00),
          &Font_5x8);
  ...
  while (expanderJoyFiltered () != JOY_LEFT)
    {
      ...
      ssd1306PrintIntAtLine (55, 1, "FL",
                             (int32_t) getTelemeterSpeed (TELEMETER_FL),
                             &Font_5x8);
      ssd1306PrintIntAtLine (55, 2, "FR",
                             (int32_t) getTelemeterSpeed (TELEMETER_DL),
                             &Font_5x8);
      ssd1306PrintIntAtLine (55, 3, "DL",
                             (int32_t) getTelemeterSpeed (TELEMETER_DR),
                             &Font_5x8);
      ssd1306PrintIntAtLine (55, 4, "DR",
                             (int32_t) getTelemeterSpeed (TELEMETER_FR),
                             &Font_5x8);
      ssd1306Refresh ();
    }
  telemetersStop ();
