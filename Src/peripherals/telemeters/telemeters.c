...
void telemeters_ADC2_IT(void)
{
    ...
    end: telemeters.end_of_conversion++;
}
...
void telemetersGetCalibrationValues(void)
{
    ...
    {
        bluetoothWaitReady();
        bluetoothPrintf("%d; %d; %d; %d\n", telemeters_profile->front.left[i], telemeters_profile->front.right[i],
                        telemeters_profile->diag.left[i], telemeters_profile->diag.right[i]);
