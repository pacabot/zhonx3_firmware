...
int moveCell(unsigned long nb_cell, float max_speed, float end_speed)
{
    ...
    if (repositionGetFrontDist(-OFFSET_DIST) > 0.00 || repositionGetFrontDist(-OFFSET_DIST) < 0.00)
    {
        move(0, (OFFSET_DIST * 2.00) + repositionGetFrontDist(-OFFSET_DIST), max_speed, end_speed);
#ifdef DEBUG_BASIC_MOVES
        bluetoothWaitReady();
        bluetoothPrintf("MOVE CELL = MAIN_DIST + OFFSET_DIST * 2 + (FRONT OFFSET) %d\n\r", (int32_t)repositionGetFrontDist(-OFFSET_DIST));
#endif
    }
