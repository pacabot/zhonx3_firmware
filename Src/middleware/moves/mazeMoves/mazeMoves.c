...
int mazeMoveFrontAlignment(double max_speed)
{
	...
	if (getWallPresence(FRONT_WALL) == TRUE)
	{
		...
		{
			while (((getTelemeterDist(TELEMETER_FL) - getTelemeterDist(TELEMETER_FR))) > 0 )
			{
				basicMove(40, 2, 100.00, 100.00);
			}
		}
		else
		{
		...
		if (((getTelemeterDist(TELEMETER_FL) + getTelemeterDist(TELEMETER_FR)) / 2.00) < FRONT_SENSOR_DIST_FOR_CENTRING_ROBOT)
		{
			...
			while (((getTelemeterDist(TELEMETER_FL) + getTelemeterDist(TELEMETER_FR)) / 2.00) < FRONT_SENSOR_DIST_FOR_CENTRING_ROBOT)//todo add in flash
					{
